...
async function getDetailBBSData(id: number) {
  const response = await fetch(`https://bbs-with-nextjs.vercel.app/api/post/${id}`, {
    cache: "no-store",
  });
...
const BBSDetailPage = async ({ params }: { params: { bbsId: number } }) => {
  ...
  return (
    <div className="mx-auto max-w-4xl p-4">
      <div className="mb-8">
        <h1 className="text-2xl font-bold">タイトル：{title}</h1>
        <p className="text-gray-700 mt-6">ユーザー名：{username}</p>
      </div>
      <div className="mb-8">
